...
function me.OnUpdate()
	if me.vars.Boss1_timer then -- PhaseChange timer; Boss 1
		...
		if tmr > me.vars.timerstate[state] then
			...
			ev.SendMessage(ev.helper.ReturnLoca("phasechange_time_message", 31-math.floor(tmr)), nil, {warning=true})
...
function me.SendSystemMsg(txt)
	...
	if  string.find(txt, me.strings.FORMATION1) then
		me.vars.Boss1_timer = {time=GetTime(), state=0} 
		SendChatMessage("Lighting!", "SAY") -- Change formation: lightning! - Boss 1
	elseif	string.find(txt, me.strings.FORMATION2) then
		me.vars.Boss1_timer = {time=GetTime(), state=0}
		SendChatMessage("Traction!", "SAY") -- Change formation: traction! - Boss 1
	elseif	string.find(txt, me.strings.FORMATION3) then  
		me.vars.Boss1_timer = {time=GetTime(), state=0} 
		SendChatMessage("Hammer!", "SAY")-- Change formation: hammer!  - Boss 1
	elseif	string.find(txt, me.strings.FORMATION4) then -- Change formation: wedge! - Boss 1
		me.vars.Boss1_timer = {time=GetTime(), state=0}
		SendChatMessage("Wedge!", "SAY")
